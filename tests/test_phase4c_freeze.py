"""Phase-4C pre-commit freeze gate (PROMPT Fase 4C section 21).

Supersedes ``tests/test_phase4b_freeze.py`` as the *live* freeze gate,
for the same reason that file itself superseded
``tests/test_phase4a_freeze.py`` in Fase 4B: the artifact it certified
moved out from under it (an external commit absorbed its own declared
uncommitted set), and this phase's own corrective fixes changed the
content-assignment ledger it had certified regardless. Both
``phase-4a-freeze.json`` and ``phase-4b-freeze.json`` are tracked here as
historical snapshots, preserved byte-for-byte; only this file compares
``phase-4c-freeze.json`` against live git/disk state - never two "active"
freezes at once.
"""
...
def test_freeze_declares_it_supersedes_the_phase_4b_freeze():
    freeze = _load()
    assert freeze["supersedes"] == "data/manifests/phase-4b-freeze.json"
    assert {e["phase"] for e in freeze["historical_freezes"]} == {"4a", "4b"}
    for entry in freeze["historical_freezes"]:
        assert entry["status"] == "superseded"


@pytest.mark.parametrize("phase", ["4a", "4b"])
def test_historical_freeze_entry_hash_matches_the_real_file_unedited(phase):
    # Neither historical freeze must ever be silently rewritten (PROMPT
    # section 20/24) - the hash this newer freeze recorded for each must
    # still match the real file on disk, byte for byte.
    freeze = _load()
...
def test_no_manifest_appeared_that_the_freeze_never_inventoried():
    ...
    on_disk = {
        ...
        if p.is_file() and p.name != "phase-4c-freeze.json"
    }
...
def test_content_assignment_ledger_assignment_ids_are_geometry_derived():
    # PROMPT Fase 4C section 8/9 - the whole point of this phase's own
    # corrective fix: re-verified here, permanently, not just once during
    # the review itself.
    import re
...
def test_git_status_porcelain_first_line_path_is_never_truncated_by_a_whole_blob_strip():
    ...
    forward once more (PROMPT Fase 4C section 19: "cobrem a classe do
    'bug do primeiro caractere do caminho'"). Deterministic, synthetic
